...
export default function Lint() {
  const { data, isLoading } = useQuery({ queryKey: ["lintClaude"], queryFn: api.lintClaude });
  ...
  return (
    <div className="animate-fade-up">
      <PageHeader title="Lint" count={files.length} />
      {files.length === 0 ? (
        <EmptyState
          icon={<FileCode size={48} />}
          title="No CLAUDE.md files found"
          description="No CLAUDE.md files were found in repos or global config."
        />
      ) : (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <div className="font-mono text-sm text-[#e2e8f0]">{file.lines}</div>
                  <div className="text-[10px] uppercase tracking-wider text-[#64748b]">lines</div>
                </div>
                <div className="text-right">
